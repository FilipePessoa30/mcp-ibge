"""Servidor MCP `mcp-saude` (scaffold/planejamento).

Status: apenas a tool `status` está registrada — este módulo existe para que
o pacote seja executável e testável desde já (`uv run mcp-saude`), servindo de
base para as tools descritas em `docs/modules/saude.md`.
...
Execução:
    python -m mcp_saude.server

Por padrão usa o transporte stdio. O transporte pode ser trocado via a
variável de ambiente `MCP_SAUDE_TRANSPORT` (ex.: "streamable-http"); host
e porta usados nesse modo são configuráveis via
`MCP_SAUDE_HOST`/`MCP_SAUDE_PORT`.
...
mcp = FastMCP(
    "mcp-saude",
    instructions=(
        "Servidor MCP (em planejamento) para dados públicos de saúde do Brasil (DataSUS / "
        "Ministério da Saúde): estabelecimentos de saúde (CNES) e indicadores por município/UF. "
...
def main() -> None:
    ...
    logging.getLogger(__name__).info(
        "Iniciando mcp-saude (transporte=%s, scaffold)",
        _settings.transport,
    )
