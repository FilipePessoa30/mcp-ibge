"""Servidor MCP `mcp-tesouro` (scaffold/planejamento).

Status: apenas a tool `status` está registrada — este módulo existe para que
o pacote seja executável e testável desde já (`uv run mcp-tesouro`), servindo de
base para as tools descritas em `docs/modules/tesouro.md`.
...
Execução:
    python -m mcp_tesouro.server

Por padrão usa o transporte stdio. O transporte pode ser trocado via a
variável de ambiente `MCP_TESOURO_TRANSPORT` (ex.: "streamable-http"); host
e porta usados nesse modo são configuráveis via
`MCP_TESOURO_HOST`/`MCP_TESOURO_PORT`.
...
mcp = FastMCP(
    "mcp-tesouro",
    instructions=(
        "Servidor MCP (em planejamento) para dados fiscais do Tesouro Nacional (Tesouro "
        "Transparente / SICONFI): relatórios fiscais (RREO, RGF) e finanças de estados e "
        "municípios. Nenhuma tool de dados está disponível nesta versão (apenas `status`) — veja "
...
def main() -> None:
    ...
    logging.getLogger(__name__).info(
        "Iniciando mcp-tesouro (transporte=%s, scaffold)",
        _settings.transport,
    )
