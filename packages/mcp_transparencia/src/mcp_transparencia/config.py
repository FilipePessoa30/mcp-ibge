...
`docs/modules/transparencia.md` para o roadmap.

Todos os valores são ajustáveis via variáveis de ambiente com prefixo
``MCP_TRANSPARENCIA_`` (ou um arquivo ``.env`` na raiz do projeto).
"""
...
# Domínios oficiais permitidos para requisições do mcp-transparencia. Vazio por
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="MCP_TRANSPARENCIA_", env_file=".env", extra="ignore"
    )
    ...
    source_name: str = "Portal da Transparência (CGU)"
    source_url: str = "https://www.portaltransparencia.gov.br/"
