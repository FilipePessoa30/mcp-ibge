...
`docs/modules/rio.md` para o roadmap.

Todos os valores são ajustáveis via variáveis de ambiente com prefixo
``MCP_RIO_`` (ou um arquivo ``.env`` na raiz do projeto).
"""
...
# Domínios oficiais permitidos para requisições do mcp-rio. Vazio por
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="MCP_RIO_", env_file=".env", extra="ignore"
    )
    ...
    source_name: str = "Prefeitura do Rio de Janeiro - Data.Rio"
    source_url: str = "https://www.data.rio/"
